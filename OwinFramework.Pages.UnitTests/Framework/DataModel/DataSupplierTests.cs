...
namespace OwinFramework.Pages.UnitTests.Framework.DataModel
{
    ...
    public class DataSupplierTests: TestBase
    {
        ...
        public void Should_register_supplies_with_scope()
        {
            ...
            Assert.IsTrue(_dataSupplier.CanSupply(registeredDependency));
            Assert.IsTrue(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(int), "test-scope")));
            Assert.IsTrue(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(int))));

            Assert.IsFalse(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(long))));
            Assert.IsFalse(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(int), "other-scope")));
            Assert.IsFalse(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(long), "test-scope")));
            Assert.IsFalse(_dataSupplier.CanSupply(dependencyFactory.Create(typeof(long), "other-scope")));
